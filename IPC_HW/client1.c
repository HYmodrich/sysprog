...
int main(){
  ...
  if((server = msgget((key_t)1111, flg)) < 0){
		perror("msgget error");
		exit(1);
	}
  ...
  if(msgsnd(server, &sbuf, sizeof(message_buf) - sizeof(long),
		IPC_NOWAIT) < 0) {
	...
	if(pthread_create(&read_thread, NULL, receive_message,
			(void *)server) != 0){
...
void* receive_message(void *server){
	...
	while(1){
		msgrcv(server_, &received[++received_index],
			sizeof(message_buf) - sizeof(long), 4, 0);
	}
}
